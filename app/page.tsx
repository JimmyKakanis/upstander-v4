import Link from 'next/link';

export default function Home() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8 sm:p-24">
      <div className="w-full max-w-md bg-white p-8 border border-gray-200 rounded-lg shadow-sm text-center">
        <h1 className="text-2xl font-bold mb-4">Welcome to Upstander</h1>
        <p className="mb-8">Anonymous Bullying Reporting for Schools.</p>
        
        <div className="space-y-4">
            <p>
              To submit a new report, please use the unique URL provided by your school.
            </p>
            <p>
              <Link href="/report/MySchoolMVP" className="text-indigo-600 hover:text-indigo-800">
                  Example Link to a School Report Form
              </Link>
            </p>
        </div>

        <div className="mt-8 border-t pt-6">
            <p className="mb-4">
              Already submitted a report? Check its status or follow up on the conversation here:
            </p>
            <Link href="/follow-up" className="inline-block w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">
                Check Report Status
            </Link>
        </div>
      </div>
    </main>
  );
}
